...
Also, we load the required functions.

```{r  loadData, echo=TRUE, message = FALSE, collapse= TRUE}
setwd("C:/Users/crist/OneDrive/Documents/Covid19France") #change to appropiate 
source("scripts/functions_covid.R")
france_departement_covid_sf <- st_read("data/covid_data.shp")
```
...
We store the sample standard deviations- in `std_deviations` of the covariates before centering and scaling the covariates and centering the dependent variable `LnHosp`.

```{r sd, echo = TRUE, collapse= TRUE}
std_deviations <- st_drop_geometry( france_departement_covid_sf) %>%
  dplyr::select(LnPop, C3, SLivR, Work, Inac, A65pls, Emer, FDoc) %>%
  summarise_all(sd)%>%as.numeric()
```


```{r scaling, echo = TRUE, collapse= TRUE}
...
Due to the usa of SLOO the following code chunk can take more less a minute to run.
...
We also perform a t-test for the importance of the variables.
...
Finally, we present the results in two tables, one for SAR and one for SEM. Due to the random choice of permutations, the following results somewhat differ from those presented in our paper.
...
p_val_pt_coef_rrsar <- format(as.numeric(p_t_test$SAR), digits = 0, trim = F, nsmall = 4)
p_val_pt_coef_rrsem <- format(as.numeric(p_t_test$SEM), digits = 0, trim = F, nsmall = 4)
...
dt_sar <- data.frame(coefsar=c(sar[["Coef"]][,1]/std_deviations,NA,sar[["rho"]][["rho"]]),
                 ...
                 pvaluesar_ridge=c( p_val_coef_rrsar, NA,p_val_rho_rrsar),
                  ...
                  pvaluesar_perm_ttest=c( p_val_pt_coef_rrsar, NA,NA)
  ...
  dt_sem <- data.frame(coefsem=c(sem[["Coef"]][,1]/std_deviations,NA,sem[["lambda"]][["lambda"]]),
                 ...
                 coefsem_ridge=c(coef_RRSEM[["Coefficients"]]/std_deviations,coef_RRSEM[["gamma"]],
                           coef_RRSEM[["lambda"]]),
                 pvaluesem_ridge=c( p_val_coef_rrsem, NA,p_val_rho_rrsem),
               pvaluesem_halawa=c( p_value_sem, NA,NA),
                  pvaluesem_perm_ttest=c(p_val_pt_coef_rrsem, NA,NA)
               )
