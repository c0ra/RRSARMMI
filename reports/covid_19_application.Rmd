...
We start by loading spatial data `covid_data.shp` which contains 12 variables and a geometry column type Multi Polygon. The observations are collected from the 96 **départements**, which are administrative units.
...
setwd(dir = "C:/Users/crist/OneDrive/Documents/Covid19France")
source("scripts/functions_covid.R")
...
```{r SARsem, echo = TRUE, collapse=TRUE, message=FALSE, comment=FALSE}
 sar <- spatialreg::lagsarlm(model,data=data,listw_1)
 sar <-summary(sar)
 sem <- spatialreg::errorsarlm(model,data=data,listw_1)
 sem <- summary(sem)
```
...
```{r RRSARrrsem, echo = TRUE, collapse=TRUE, message=FALSE, comment=FALSE, cache=TRUE, dpi = 72}
coef_RRSAR <- rrsar(data, model, buffer, plot=TRUE)
coef_RRSEM <- rrsem(data, model, buffer, plot=TRUE)
```

We plot the coefficients path and the log-likelihood versus the ridge regularization parameter for RRSAR and RRSEM.

```{r plotResults, echo = TRUE, collapse=TRUE, message=FALSE, comment=FALSE, cache=TRUE}
...
p_val_rho_rrsar <- format(as.numeric(coef_RRSAR[["pvalue"]]), digits = 0, trim = F, nsmall = 3)
p_val_coef_rrsar <- format(as.numeric(p_F_test$SAR), digits = 0, trim = F, nsmall = 3)

p_val_rho_rrsem <- format(as.numeric(coef_RRSEM[["pvalue"]]), digits = 0, trim = F, nsmall = 3)
p_val_coef_rrsem <- format(as.numeric(p_F_test$SEM), digits = 0, trim = F, nsmall = 3)


dt <- data.frame(coefsar=c(sar[["Coef"]][-1,1],NA,sar[["rho"]][["rho"]], NA),
                 pvaluesar = c(sar[["Coef"]][-1,4],NA,as.numeric(sar[["LR1"]][["p.value"]]),NA),
                 coefsar_ridge=c(coef_RRSAR[["Coefficients"]],coef_RRSAR[["gamma"]],
                           coef_RRSAR[["rho"]],NA),
                 pvaluesar_ridge=c( p_val_coef_rrsar, NA,p_val_rho_rrsar, NA),
                 coefsem = c(sem[["Coef"]][-1,1],NA,NA,sem[["lambda"]][["lambda"]]),
                 pvaluesem = c(sem[["Coef"]][-1,4], NA, NA, as.numeric(sem[["LR1"]][["p.value"]])),
                 coefsem_ridge=c(coef_RRSEM[["Coefficients"]],coef_RRSEM[["gamma"]],
                           NA,coef_RRSEM[["lambda"]]),
                 pvaluesem_ridge=c( p_val_coef_rrsem, rep(NA,2),p_val_rho_rrsem)
               )

  rownames(dt) <-c(rownames(p_F_test),"$\\gamma$", "$\\rho$","$\\lambda$")


opts <- options(knitr.kable.NA = "")
knitr::kable(dt,  col.names = c("Coef",
                                "p-value",
                                "Coef",
                                "p-value",
                                "Coef",
