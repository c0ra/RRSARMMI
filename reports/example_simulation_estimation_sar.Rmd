...
1. Initialization. Consider the ordinary linear regression model $\mathbf{Y}=\mathbf{X}\pmb{\beta}+\pmb{\varepsilon}$ and estimate $\pmb{\beta}$ by $\hat{\pmb{\beta}}_{R}$ defined by $\hat{\pmb{\beta}}_{R}=(\mathbf{X}^T\mathbf{X}+\gamma\mathbf{I}_p)^{-1}\mathbf{X}^T\mathbf{Y}$.  
2. Consider  $\mathbf{Y}=\rho W\mathbf{Y}+\mathbf{X}\hat{\pmb{\beta}}_{R}+\pmb{\varepsilon}$, compute $\mathbf{e}_0=\mathbf{y}-\mathbf{X}\hat{\pmb{\beta}}_{R}$ and $\mathbf{e}_L=W\mathbf{y}-\mathbf{X}\hat{\pmb{\beta}}_{R}$ and estimate 
...
4. Consider $\mathbf{Y}=\rho W\mathbf{Y}+\pmb{X}\hat{\pmb{\beta}}_R^{SAR}+\pmb{\varepsilon}$, compute $\mathbf{e}_0=\mathbf{y}-\mathbf{X}\hat{\pmb{\beta}}_R^{SAR}$ and $\mathbf{e}_L=W\mathbf{y}-\mathbf{X}\hat{\pmb{\beta}}_R^{SAR}$ and come back to the 
estimation of $\rho$ by using the concentrated  ML. 
5. Repeat steps 3  and 4 (using new estimators) until convergence.
...
```{r RRSAR, echo = TRUE, collapse=TRUE, message = FALSE, message=FALSE, cache=TRUE}
