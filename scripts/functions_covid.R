...
rrsar <- function(data, model, buffer, plot = FALSE, scale=FALSE){
  ...
  for (k in 1:n){
    ...
    if(buffer>1){
      ...
      p <- ncol(test.data %>% select(predictorvarnames))
...
rrsem <- function(data, model, buffer, plot = FALSE, scale=FALSE){
  ...
  for (k in 1:n){
    ...
    if(buffer>1){
      ...
      x_train <- x[data$SLOO=='Train',]
      y_train <- y[data$SLOO=='Train']
      p <- ncol(test.data %>% select(predictorvarnames))+1
...
ridge.sloo.lr<- function(data, model, buffer, plot){
  ...
  x <- as.matrix(data %>% select(predictorvarnames))
  
  x<- scale(x, scale=FALSE) #center variables
  y <- as.matrix(data %>% select(responsevarname))
  ...
  for (k in 1:n){
    ...
    if(buffer>1){
      ...
      x_train <- as.matrix(train.data %>% select(predictorvarnames))
      y_train <- as.matrix(train.data %>% select(responsevarname))
      ...
      predmat_lr[k,] <- predict(fit, newx = as.matrix(test.data %>% select(predictorvarnames)),
                                type="response")
